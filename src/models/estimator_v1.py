...
def get_regularized_variable(name, shape=(), l2_reg=L2_REG, **kwargs):
    ...
    variables = tf.compat.v1.get_variable(
        name, shape, regularizer=regularizer, **kwargs
    )
    return variables
...
def add_activity_loss(tensor, l2_reg=L2_REG):
    ...
    tf.compat.v1.add_to_collection(
        tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, mean_activity_loss
    )
    return tensor


def get_field_values(
    features, field_values, vocab_txt=VOCAB_TXT, embedding_size=64, l2_reg=1.0
):
    ...
    with tf.name_scope(name):
        ...
        # variables
        field_embeddings = get_regularized_variable(
            name + "_embeddings", [vocab_size, embedding_size], l2_reg=0
        )
        field_biases = get_regularized_variable(
            name + "_biases", [vocab_size], l2_reg=0
        )
        ...
        field_embed = tf.nn.embedding_lookup(
            field_embeddings, field_idx, name=name + "_embed_lookup"
        )
        add_activity_loss(field_embed, l2_reg / embedding_size)
        # [None, embedding_size]
        field_bias = tf.nn.embedding_lookup(
            field_biases, field_idx, name=name + "_bias_lookup"
        )
...
def get_similarity(field_values, vocab_txt=VOCAB_TXT, top_k=TOP_K):
    ...
    with tf.name_scope(name):
        ...
        field_cosine_sim = cosine_similarity(
            field_values["embed"], field_values["embeddings"]
        )
        # [None, mapping_size]
        field_top_k = tf.math.top_k(field_cosine_sim, k=top_k, name="top_k_sim_" + name)
        field_top_k_sim, field_top_k_idx = field_top_k
        # [None, k], [None, k]
        field_top_k_string = id_string_table.lookup(
            tf.cast(field_top_k_idx, tf.int64), name=name + "_string_lookup"
        )
        # [None, k]
    field_values.update(
        {"top_k_sim": field_top_k_sim, "top_k_string": field_top_k_string}
    )
    return field_values
...
def model_fn(features, labels, mode, params):
    ...
    with tf.name_scope("mf"):
        # global bias
        global_bias = get_regularized_variable(
            "global_bias", initializer=tf.zeros_initializer, l2_reg=0
        )
        # []
        tf.compat.v1.summary.scalar("global_bias", global_bias)
        # row mapping, embeddings and biases
        row_values = get_field_values(
            features, row_values, vocab_txt, embedding_size, l2_reg
        )
        # column mapping, embeddings and biases
        col_values = get_field_values(
            features, col_values, vocab_txt, embedding_size, l2_reg
        )

        # matrix factorisation
        embed_product = tf.reduce_sum(
            tf.multiply(row_values["embed"], col_values["embed"]), 1
        )
        # [None, 1]
        logits = tf.add_n(
            [
