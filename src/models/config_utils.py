...
def init_params(params):
    ...
    if not params["disable_datetime_path"]:
        ...
        job_dir = "{job_dir}-{datetime:%Y%m%d-%H%M%S}".format(
            job_dir=params["job_dir"], datetime=datetime_now
        )
        params["job_dir"] = job_dir
    tf.io.gfile.makedirs(params["job_dir"])

    # vocab_txt
    output_vocab_txt = os.path.join(
        params["job_dir"], os.path.basename(params["vocab_txt"])
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--row-name", default=ROW_NAME, help="row id name (default: %(default)s)"
    )
    parser.add_argument(
        "--col-name", default=COL_NAME, help="column id name (default: %(default)s)"
    )
    parser.add_argument(
        "--target-name", default=TARGET_NAME, help="target name (default: %(default)s)"
    )
    parser.add_argument(
        "--weight-name", default=WEIGHT_NAME, help="weight name (default: %(default)s)"
    )
    parser.add_argument(
        "--pos-name", default=POS_NAME, help="positive name (default: %(default)s)"
    )
    parser.add_argument(
        "--neg-name", default=NEG_NAME, help="negative name (default: %(default)s)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--optimizer", default=OPTIMIZER, help="name of optimzer (default: %(default)s)"
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
    ...
    logger.info(
        "call: %s.", " ".join(sys.argv), extra={"ArgumentParser": args.__dict__}
    )
