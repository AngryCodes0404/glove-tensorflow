...
def build_compile_model(params):
    ...
    inputs = [
        ...
        for name in [params["row_name"], params["col_name"]]
    ]
...
def get_glove_dataset(row_col_names, vocab_txt=VOCAB_TXT, **kwargs):
    ...
    def lookup(features, targets, weights):
        features = {
            name: string_id_table.lookup(features[name], name=name + "_lookup")
...
def main():
    params = parse_args()
    params["serving_input_fn_args"] = {
        "int_features": [params["row_name"], params["col_name"]]
    }
    save_params(params)
